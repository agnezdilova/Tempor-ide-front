...
def create_container():
    if (HOST == 'localhost'):
        create_cmd = f'docker create --publish 3000 {DOCKER_IMAGE}'
    else:
    	create_cmd = f'docker create --ip={HOST} --publish 3000 {DOCKER_IMAGE}'
    	
    current_app.logger.info(create_cmd)
    id = run_cmd(create_cmd)[:12]
    current_app.logger.info('docker ps -a --filter "id=' + id + '" --format "{{.Names}}"')
